...
while people_waiting_for_lift:
    if sum(current_state_of_lift) == wagon_capacity * len(current_state_of_lift):
    ...
    if not people_waiting_for_lift and sum(current_state_of_lift) == wagon_capacity * len(current_state_of_lift):
        print(" ".join(str(x) for x in current_state_of_lift))
        break
else:
